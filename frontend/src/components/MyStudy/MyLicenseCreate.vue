<template>
  <div>
    <v-btn @click="openForm" class="mx-2" fab dark color="indigo"><v-icon dark>mdi-plus</v-icon></v-btn>
    <v-card v-if="showForm">
      ...
      <v-card-actions>
        ...
        <v-btn color="blue darken-1" text @click="closeForm">Save</v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
  ...
  watch: {
    'updateData': function(){
      if(this.updateData) {
        ...
        this.myLicenseData = this.updateData
        this.openForm()
      }
    }
  },

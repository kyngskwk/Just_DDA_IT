<template>
  <v-card
    v-if="isShow"
    class="my-2 rounded-xl mylicense"
    outlined
    style="border-width: 0.1rem; background-color:white;"
  >
    <v-card-subtitle
      v-if="isFin && LicenseData.dueDate"
      class="pl-5 pb-0 font_k"
      style="color:#4DB6AC;"
    >※ 만료된 자격증</v-card-subtitle>
    <div class="d-flex flex-row align-center justify-space-between pl-8 pr-2">
      <div class="d-flex flex-row justify-start align-center">
        <v-card-title class="px-0 font_l_k">{{ passLicense.license.licenseName }}</v-card-title>
        <div>
          <v-card-subtitle
            class="font_k"
            v-if="passLicense.serialNumber"
          >{{ passLicense.serialNumber }}</v-card-subtitle>
      ...
      <div class="d-flex flex-row justify-end align-center">
        ...
        <div>
          <v-card-subtitle
            v-if="passLicense.licenseGrade"
            class="font_k"
            style="color:black;"
          >{{ passLicense.licenseGrade }}등급</v-card-subtitle>
          <v-card-subtitle
            v-if="passLicense.licenseScore != '0'"
            class="font_k"
            style="color:black;"
          >{{ passLicense.licenseScore }}점</v-card-subtitle>
          <v-card-subtitle
            v-if="passLicense.licenseScore == '0' && !passLicense.licenseGrade"
            class="font_k"
            style="color:black;"
          >점수/등급</v-card-subtitle>
        </div>
        <!-- 수정 삭제 메뉴 -->
    ...
    <div class="d-flex flex-row justify-space-between align-start">
      <div>
        <v-card-subtitle
          class="pt-0 pl-8 font_k"
          style="color:black;"
          v-if="passLicense.gainDate || passLicense.dueDate"
        >{{ passLicense.gainDate }}~{{ passLicense.dueDate }}</v-card-subtitle>
        <v-card-subtitle
          class="pt-0 pl-8 font_k"
          style="color:black;"
          v-if="!passLicense.dueDate && !passLicense.gainDate"
        >날짜를 입력해주세요.</v-card-subtitle>
      </div>
    </div>
  </v-card>
  <!-- <tr>

...
export default {
  ...
  methods: {
    ...
    del() {
      ...
      axios.post(`http://${this.$store.state.address}:${this.$store.state.port}/feed/deleteComment`, comment)
      .then(response => {
    ...
    update() {
      ...
      // console.log(comment)
      axios.post(`http://${this.$store.state.address}:${this.$store.state.port}/feed/updateComment`, comment, {
        params: {
          'feedId': this.feedId,
          'UID': this.UID
        }

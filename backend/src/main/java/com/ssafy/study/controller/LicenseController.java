...
public class LicenseController {
	...
	public Object deleteReview(@RequestParam Long id, HttpSession session){
		ResponseEntity response = null;
		BasicResponse result = new BasicResponse();

		Optional<LicenseReview> review = reviewRepo.findById(id);
		if(!review.isPresent()){
			result.status = false;
			result.data = "리뷰 정보 없음";
			response= new ResponseEntity<>(result,HttpStatus.NOT_FOUND);
		}
	...
	public Object addMyLicense(@RequestBody createMyLicenseDTO mylicenseObject, HttpSession session){
		ResponseEntity response = null;
		BasicResponse result = new BasicResponse();
		System.out.println(mylicenseObject.getUID());
		...
		Optional<MyLicense> exist;
		if(mylicenseObject.getId()!=null){
			exist = mylicenseRepo.findById(mylicenseObject.getId());
		}else{
			exist = mylicenseRepo.findByMemberAndLicenseAndLicenseStatus(member.get(),license.get(),mylicenseObject.getLicenseStatus());
		}

		if(exist.isPresent()) {
			exist.get().setLicenseStatus(mylicenseObject.getLicenseStatus());
			exist.get().setLicenseScore(mylicenseObject.getLicenseScore());
			exist.get().setLicenseGrade(mylicenseObject.getLicenseGrade());
			exist.get().setDueDate(mylicenseObject.getDueDate());
			exist.get().setTestDate(mylicenseObject.getTestDate());
			exist.get().setGainDate(mylicenseObject.getGainDate());
			exist.get().setSerialNumber(mylicenseObject.getSerialNumber());
			mylicenseRepo.save(exist.get());
			result.object=exist.get();
		}else {
